                    ...
                    <div class="table-responsive">
                        <table id="data-table-basic" class="table table-striped">
                            ...
                            @foreach($contracts as $index => $c)
                                <tr>
                                    <td>{{ $index+1 }}</td>
                                    <td>{{ $c->car['0']->mark }}/{{ $c->car['0']->model }}</td>
                                    <td>{{ $c->client['0']->fullname }}</td>
                                    <td>{{ $c->employee['0']->fullname }}</td>
                                    <td>{{ $c->start_of_contract}}</td>
                                    <td>{{ $c->end_of_contract}}</td>
                                    <td>{{ $c->price}}</td>
                                    <td>
                                        <div class="row sweetalert justify-content-center">
                                            <div>
                                                <button class="edit-arrival" data-toggle="modal" data-target="#edit-arrival{{$c['contracts_id']}}" title="Edit" type="button" style="border: 0; background:0">
                                                    <span class="material-icons">&#xe8ff;</span>
                                                </button>
                                            </div>
                                            <div>
                                                <form method="POST"
                                                      action="{{route('contracts.delete', ['contracts' => $c['contracts_id']])}}"> @csrf
                                                <button title="Delete" data-toggle="tooltip" type="submit" name = "contracts" id="contracts" style="border: 0; background:0">
                                                    <span class="material-icons">&#xe92b;</span>
                                                </button>
                                                </form>
                                            </div>
